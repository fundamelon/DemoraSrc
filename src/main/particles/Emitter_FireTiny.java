...
public class Emitter_FireTiny extends ParticleEmitter {
	 ...
	 */
	public Emitter_FireTiny(int x, int y) {
		this.x = x;
		this.y = y;
	}
	 ...
	 */
	public Emitter_FireTiny(int x, int y, float size) {
		this.x = x;
		this.y = y;
		this.size = size;
	}
	...
	public void update(ParticleSystem system, int delta) {
		...
		if (timer <= 0) {
			...
			p.setPosition(x, y);
			p.setSize(size * 0.01f);
			float vx = (float) (-0.005f + (Math.random() * 0.01f))*0.2f + (float)Math.sin(variation_timer * 0.0003 + Math.sin(variation_timer*0.001))*0.003f;
			float vy = (float) (-0.01f-(Math.random() * 0.02f));
	...
	public void updateParticle(Particle particle, int delta) {
		if (particle.getLife() > 600) {
			particle.adjustSize(0.04f * delta);
		} else {
			particle.adjustSize(0.9f * delta * (size / 40.0f));
		}
