...
public class Emitter_Magic_BallMed extends ParticleEmitter {
	 ...
	 */
	public Emitter_Magic_BallMed(int x, int y) {
		this.x = x;
		this.y = y;
	}
	 ...
	 */
	public Emitter_Magic_BallMed(int x, int y, float size) {
		this.x = x;
		this.y = y;
		this.size = size;
	}
...
//				float vy = (float) (-0.5f + Math.random())*0.1f;
				...
				float vx = (float)Math.cos(ang) * 0.1f;
				float vy = (float)Math.sin(ang) * 0.1f;
