...
public class Emitter_UmbrinSmoke_1 extends ParticleEmitter {
	 ...
	 */
	public Emitter_UmbrinSmoke_1(int x, int y, int z) {
		this.x = x;
		this.y = y;
		this.z = z;
	}
	 ...
	 */
	public Emitter_UmbrinSmoke_1(int x, int y, int z, float size) {
		this.x = x;
		this.y = y;
		this.z = z;
		this.size = size;
	...
	public Emitter_UmbrinSmoke_1(float x, float y, float z) {
	...
	public void update(ParticleSystem system, int delta) {
		...
		if (timer <= 0 && isEnabled() && variation_timer <= 100) {
			for(int i = 0; i < 30; i++) {
				...
				p.setPosition(x + 15f * (float)(Math.random() - 0.5f), y + (float)(Math.random() * 1f - 0.5f), z + (float)(Math.random() * 70f + 3f));
				p.setSize(size * (float)(Math.random()*0.9f + 0.5f));
				float ang = (float) Math.random() * 360;
				
				vel_x = (float)Math.cos(ang) * 0.1f * ((float)Math.random()) * 15f;
				vel_y = (float)Math.cos(ang) * 0.1f * ((float)Math.random()) * 0.1f;
				vel_z = Math.abs((float)Math.sin(ang) * 0.1f * ((float)Math.random()) * 20f);
				
				p.setVelocity(vel_x, vel_y, vel_z, 0.05f);
			}
		}
	}
