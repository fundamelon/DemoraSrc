...
public class Particle {
	...
	public void render() {
		...
		//			(int) size, (int) size);
			GraphicsManager.submitToQueue(image.getScaledCopy((int)size, (int)size), 
					(int) (x - (size / 2)), 
					(int) (y - (size / 2)) - (int) (z), 
					color, 
					this.getY());
	...
	public void setVelocity(float dirx, float diry, float dirz, float speed) {
		this.setVelocity(dirx,  diry, speed);
		this.velz = dirz * speed;
	}
